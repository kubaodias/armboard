...
int main(void)
{
  ...
  int rombootSizeAddr = 0x00203000;
  int loadRomboot = *(int *)rombootSizeAddr;

  //printf("load?: %x, size: %d\n", loadRomboot, rombootSize);
  if (loadRomboot == 0x10adb007)
  {
	  SizeToDownload = *(int *)(rombootSizeAddr + 4);
	  AddressToDownload = 0x00200000;
	  ...
	  *(int *)(AddressToDownload + AT91C_OFFSET_VECT6) = i;
	  ...
	  printf("\n\rModification of Arm Vector 6 :0x%x\n\r", i);
	  ...
	  printf("dataflash to be written at %x from %x, size: %d\n\r", DeviceAddress, AddressToDownload, SizeToDownload);
	  // write the dataflash
	  write_dataflash (DeviceAddress, AddressToDownload, SizeToDownload);
	  printf("dataflash written at %x from %x, size: %d\n\r", DeviceAddress, AddressToDownload, SizeToDownload);
  ...
  //AT91F_MemoryDisplay(DeviceAddress, 4, 16);
  printf("odys loader 1.0\n\rpress any key\n\r");
  getc();
  // stop tempo
